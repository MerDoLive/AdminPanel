...
error_reporting(1);
class Categories_level3 extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('Categories_level3_model');
	...
	{
		// roshani code start
		$all=$this->Categories_level3_model->count_filtered('ALL');
        $live=$this->Categories_level3_model->count_filtered('LIVE');
        $pndg=$this->Categories_level3_model->count_filtered('PNDG');
        $rej=$this->Categories_level3_model->count_filtered('REJ');
        $deleted=$this->Categories_level3_model->count_filtered('DLTD');
		$data = array('heading'=>'Category Level 3', 'cond2'=>$cond2,'all'=>$all,'live'=>$live,'deleted'=>$deleted,'pndg'=>$pndg,'rej'=>$rej);
		//roshani code end
		$this->load->view('category_level3/list',$data);
	}

	public function ajax_manage_page($cond2)
	{
		$categoryData = $this->Categories_level3_model->get_datatables($cond2);
		...
		foreach ($categoryData as $catData) 
		{
            ...
            {
			$btn = anchor(site_url('Categories_level3/update/'.$cond2.'/'.$catData->CATG_MSTR_CATEGORY_ID),'<button title="Edit" class="btn btn-sm btn-info waves-effect"><i class="zmdi zmdi-edit"></i></button>');
			$btn .= ' | '.anchor(site_url('Categories_level3/view/'.$cond2.'/'.$catData->CATG_MSTR_CATEGORY_ID),'<button title="View" class="btn btn-sm btn-info waves-effect"><i class="zmdi zmdi-eye"></i></button> | ');
		}
			 if($catData->CATG_MSTR_STATUS=='DLTD')
            {
            $btn .= anchor(site_url('Categories_level3/restore/'.$cond2.'/'.$catData->CATG_MSTR_CATEGORY_ID),'<button onclick="javasciprt: return confirm(\'Are you really want to Restore?\')"title="Restore" class="btn btn-sm btn-info waves-effect"><i class="zmdi zmdi-time-restore"></i></button>&nbsp;');
        }else
        {
            $btn .= anchor(site_url('Categories_level3/delete/'.$cond2.'/'.$catData->CATG_MSTR_CATEGORY_ID),'<button onclick="javasciprt: return confirm(\'Are you really want to delete?\')" title="Delete" class="btn btn-sm btn-danger waves-effect"><i class="zmdi zmdi-delete"></i></button>');
        }
			...
			/*$nestedData[] = ucfirst($parentCatName);*/
		...
		$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->Categories_level3_model->count_all($cond2),
				"recordsFiltered" => $this->Categories_level3_model->count_filtered($cond2),
				"data" => $data,
			       );
	...
	public function create($cond2)
	{
		...
		$allCategoryLevel = $this->Crud_model->GetData('CATG_MSTR','',"CATG_MSTR_STATUS='LIVE' and  	CATG_MSTR_CATEGORY_LEVEL='2'",'','');
		$data = array('heading'=>'Category',
				'subheading'=>'Create Category Level 3',
				'button'=>'Submit',
				'action'=>site_url('Categories_level3/create_action'),
				'categories' => $allCategoryType,
				'mainCategory' => $allCategory,
				'mainCategorylevel' => $allCategoryLevel,
		...
		$this->load->view('category_level3/form',$data);
	}
	...
	public function create_action() 
	{
		...
		if ($this->form_validation->run() == FALSE) 
		{
			...
			$data = array('heading'=>'Category',
				...
				'mainCategoryId1' => $_POST['mainParentcategoryId'],
				'name' => $_POST['name'],
				'desc' => $_POST['desc'],
				'id' => set_value('CATG_MSTR_CATEGORY_ID'),
			...
			if($_POST['maincategoryId']!='')
					{
						...
						$this->session->set_flashdata('message', '<div class="alert alert-block alert-success"><p>Record has been added successfully.</p></div>');
						        redirect(site_url('Categories_level3/index/'.$_POST['cond2']));
				}
				else
				{
					$allCategoryType = $this->Crud_model->GetData('CATG_TYPE_MSTR','',"CATG_TYPE_MSTR_STATUS='LIVE'",'','');
					...
					$this->session->set_flashdata('message', '<div class="alert alert-block alert-danger"><p>Provide Category Level2, found empty </p></div>');
						$this->load->view('category_level3/form',$data);
				}
	...
	public function update($cond2,$id)
	{
		...
		$getMapCategory1 = $this->Crud_model->GetData('CATG_SUB_CATG_MAP','CATG_SUB_CATG_MAP_PARENT_CATEGORY_ID',"CATG_SUB_CATG_MAP_CATEGORY_ID='".$getMapCategory->CATG_SUB_CATG_MAP_PARENT_CATEGORY_ID."'",'','','','single');
		$data = array('heading'=>'Categories',
				'subheading'=>'Update Category Level 3',
				'button'=>'Update',
				'categories' => $allCategoryType,
				'mainCategory' => $allCategory,
				'mainCategorylevel' => $allCategoryLevel,
				'action'=>site_url('Categories_level3/update_action'),
				'mainCategoryId' => set_value('mainCategoryId',$getMapCategory->CATG_SUB_CATG_MAP_PARENT_CATEGORY_ID),
				'mainCategoryId1' => set_value('mainCategoryId1',$getMapCategory1->CATG_SUB_CATG_MAP_PARENT_CATEGORY_ID),
	...
	public function delete($cond2,$id)
	{
       ...
       // $this->Crud_model->DeleteData('CATG_TYPE_MSTR',"CATG_TYPE_MSTR_CATEGORY_TYPE_ID='".$id."'");
		...
		redirect('Categories_level3/index/cond2');
	}
	...
	public function view($cond2,$id){
		...
		$data = array(
				'heading'=>'Categories',
				'subheading'=>'View Category Level 3',
				//'name' => set_value('name',$getCategoryType->CATG_TYPE_MSTR_CATEGORY_TYPE),
				...
				'categoryLevel3' => set_value('categoryLevel3',$getCategory2->CATG_MSTR_CATEGORY_NAME),
				'position' => set_value('position',$getCategory->CATG_MSTR_POSITION),
		...
		$this->load->view('category_level3/view',$data);
	 ...
	 public function restore($cond2,$id)
    {
       ...
       // $this->Crud_model->DeleteData('CATG_TYPE_MSTR',"CATG_TYPE_MSTR_CATEGORY_TYPE_ID='".$id."'");
        ...
        {
        $this->session->set_flashdata('message', '<div class="alert alert-block alert-danger"><p>Record unable to restore because mapped category level 2 not available.</p></div>');
        }
    ...
    public function export()
    {
        ...
        $table_columns=array("Category Level 3 ID","Category Level 3","Category Type","Category Level 2","Status","Created By","Created Time","Modified By","Modified Time");
        $column=0;
        ...
        foreach($categoryData as $row)
        {
            ...
            $object->getActiveSheet()->setCellValueByColumnAndRow(5,$excel_row,$row->CATG_MSTR_CREATED_BY);
            $object->getActiveSheet()->setCellValueByColumnAndRow(6,$excel_row,$row->CATG_MSTR_CREATED_TIME);
            $object->getActiveSheet()->setCellValueByColumnAndRow(7,$excel_row,$row->CATG_MSTR_MODIFIED_BY);
            $object->getActiveSheet()->setCellValueByColumnAndRow(8,$excel_row,$row->CATG_MSTR_MODIFIED_TIME);
            $excel_row++;
        }
        $object_writer=PHPExcel_IOFactory::createWriter($object,'Excel5');
        header('Content-Type:application/vnd.ms-excel');
        header('Content-Disposition:attachment;filename="CategoryLevel3-'.date('d-m-y-h:m:s').'.xls"');
